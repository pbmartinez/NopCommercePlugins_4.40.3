...
namespace Nop.Web.Framework.Infrastructure.Extensions
{
    ...
    public static class ApplicationBuilderExtensions
    {
        ...
        public static void UseNopStaticFiles(this IApplicationBuilder application)
        {
            ...
            // Support for unrecognized files type
            // For Letsecnrypt http challenge
            var environment = (IWebHostEnvironment)application.ApplicationServices.GetService(typeof(IWebHostEnvironment));
            var wwwroot = environment.WebRootPath;
            var wellKnown = Path.Combine(wwwroot, ".well-known");
            var exists = Directory.Exists(wellKnown);
            if (!exists)
            {
                Directory.CreateDirectory(wellKnown);
            }
            exists = Directory.Exists(wellKnown);
